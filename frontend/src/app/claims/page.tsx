...
export default function ClaimsDashboard() {
  const claims = [
    { id: "CL-8192", date: "2026-03-10", pool: "ETH / USDT", amount: "$12.42", status: "Settled", icon: <CheckCircle2 className="text-green-400" /> },
    { id: "CL-8185", date: "2026-03-09", pool: "WBTC / ETH", amount: "$42.10", status: "Ready to Claim", icon: <Zap className="text-aegis-accent animate-pulse" /> },
    { id: "CL-8172", date: "2026-03-05", pool: "ETH / USDT", amount: "$4.80", status: "Settled", icon: <CheckCircle2 className="text-green-400" /> },
  ]
                ...
                <div key={claim.id} className="grid grid-cols-5 p-8 items-center group hover:bg-white/[0.01] transition-all">
                  <div className="col-span-2 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-aegis-border p-2">
                      <img 
                        src={claim.pool.includes("ETH") ? "https://cryptologos.cc/logos/ethereum-eth-logo.svg" : "https://cryptologos.cc/logos/wrapped-bitcoin-wbtc-logo.svg"} 
                        alt="Logo" 
                    ...
                    <div>
                      ...
                      <div className="text-[10px] font-bold text-aegis-text-dim uppercase">{claim.date} • ID: {claim.id}</div>
                    </div>
                  </div>
                  <div className="text-right font-black text-lg">{claim.amount}</div>
                  <div className="text-right flex flex-col items-end">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                      claim.status === "Settled" ? "bg-green-500/10 text-green-400" : "bg-aegis-accent/10 text-aegis-accent"
                    }`}>
                      {claim.status}
                    </span>
                  </div>
                  <div className="text-right">
                    {claim.status === "Settled" ? (
                      ...
                      <button className="px-6 py-2 rounded-xl accent-gradient text-black font-black text-[10px] glow-accent transition-all hover:scale-105">
                        SETTLE NOW
                      </button>
                    )}
                  </div>
        ...
        <div className="space-y-8">
          ...
          <div className="glass-card p-8 rounded-[40px] border border-amber-500/20 bg-amber-500/[0.02]">
            ...
            <p className="text-xs font-bold text-aegis-text-dim leading-relaxed mb-6">
              ETH/USDT liquidity is experiencing high volatility. Aegis dynamic fees have been scaled to 0.45% and premiums adjusted (+15%) by the Reactive Network to maintain reserve health.
            </p>
